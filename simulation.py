...
def sim1(m, n, p, H):
    ...
    graphs = [erd_ren(n, p) for i in range(m)]
    XbarH = 0
    for l in range(m):
        M = graphs[l]
        XHMl = 0
        ...
        pool = mp.Pool(mp.cpu_count())
        results = pool.starmap(algorithmOne, [(H, M, rand_assign(K, len(M)))
                                                        for i in range(t)])
        pool.close()
...
def sim1_err(exp_val, rec_val):
    # returns error from expected and received values
    return 1 - exp_val / rec_val

def checksim1():
    # big function: runs sim1 and compares to expected

    args = [3, 100, 0.5, [0, 1, 1]]

    rec_val = sim1(*args)
    print(rec_val)
    exp_val = exp_sim1(*args)
    print(exp_val)
    dif = sim1_err(exp_val, rec_val)
    print(dif)
...
def sim1_many():
    ...
    for i in lst:
        ...
        rec_val = sim1(*args)
        exp_sim = exp_sim1(*args)
        err = sim1_err(exp_sim, rec_val)

        temp_lst.append(rec_val)
        temp_lst.append(exp_sim)
        temp_lst.append(err)
...
def run_Y_comp(n, p, s, K, Corr):
    ...
    A_center = center(A, p * s)
    B_center = center(B, p * s)
    Y_corr = algorithmTwo(A_center, B_center, K)
    comp_corr = 0.5 * r * r * math.factorial(n) / math.factorial(n - K - 1)
    comp_corr *= math.pow((p * s * s * (1 - p)), K) * len(T)
    print('rec_Y', Y_corr)
    print('exp_Y', comp_corr)
    if Y_corr >= comp_corr:
        return 1
    else:
        return 0
